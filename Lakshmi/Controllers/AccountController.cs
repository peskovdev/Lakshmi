    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> DeleteConfirmed()
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    return PartialView("DeleteAcc");
                }
            }
            return RedirectToAction("Index", "Home");
        }
        public ActionResult DeleteUs(string id) //Удалить фото
        {
            ...
            if (b.Id == User.Identity.GetUserId())
            {
                return Content("Свой Аккаунт вы можете удалить в настройках вашего аккаунта");
            }
            if (User.IsInRole("Admin") || User.IsInRole("Moder"))
            {
                return PartialView(b);
            }
            return Content("Вы не можете удалить не свой аккаунт, для данной операции вы должны обладать правами модератора");
        }
        [HttpPost, ActionName("DeleteUs")]
        public ActionResult DeleteConfirmedUs(string id)
        {
            ApplicationUser b = db.Users.Find(id);
            if (b == null)
            {
